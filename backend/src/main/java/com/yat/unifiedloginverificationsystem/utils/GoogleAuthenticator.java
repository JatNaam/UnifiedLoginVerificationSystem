package com.yat.unifiedloginverificationsystem.utils;
...
public class GoogleAuthenticator {
    ...
    public static String getQrCodeText(String secretKey, String account, String issuer) {
        ...
        try {
            return "otpauth://totp/"
                    + URLEncoder.encode((!StringUtils.isEmpty(issuer) ? (issuer + ":") : "")
                    + account, "UTF-8").replace("+", "%20")
                    + "?secret="
                    + URLEncoder.encode(normalizedBase32Key, "UTF-8").replace("+", "%20")
                    + (!StringUtils.isEmpty(issuer) ? ("&issuer="
